<template>
  ...
  <el-card style="margin: 10px 0px">
    <el-button type="primary" size="default" @click="addUser" style="margin: 0 8px" v-has="`btn.User.add`">
      添加用户
    </el-button>
    <!-- 添加更新用户抽屉 -->
    <el-drawer v-model="drawer">
      ...
      <template #footer>
        ...
        <el-button type="primary" size="default" @click="saveUser">
          确定
        </el-button>
      </template>
    </el-drawer>

    <el-button type="primary" size="default" :disabled="selectIdArr.length ? false : true" @click="deleteList">
      批量删除
    </el-button>
    <!-- table展示用户信息 -->
      ...
      <el-table-column label="操作" width="300px" align="center">
        <template #="{ row }">
          ...
          <el-popconfirm :title="`你确定要删除${row.username}?`" width="260px" @confirm="deleteUser(row.id)">
            <template #reference>
              <el-button type="primary" size="small" icon="Delete" v-has="`btn.User.remove`">
                删除
              </el-button>
            </template>
          </el-popconfirm>
    ...
    <template #default>
      <el-form>
        ...
        <el-form-item label="角色列表">
          ...
          <el-checkbox-group v-model="userRoleList" @change="handleCheckedCitiesChange">
            <el-checkbox v-for="(role, index) in allRole" :key="index" :label="role.roleName" :value="role.id">
              {{ role.roleName }}
            </el-checkbox>
          </el-checkbox-group>
        </el-form-item>
...
let allRoleIdList = ref<number[]>([])
let userRoleList = ref<number[]>([])
...
const getHasUser = async (pager = 1) => {
  //收集当前页码
  pageNo.value = pager
  let result: UserResponseData = await reqUserDataInfo(
    pageNo.value,
    pageSize.value,
    keyword.value,
  )
...
const setRole = async (row: UserData) => {
  ...
  if (result.code == 200) {
    ...
    //存储全部角色的信息
    allRoleIdList.value = result.data.allRolesList.map(item => item.id as number)
    //存储当前用户已有角色的id
    userRoleList.value = result.data.assignRoles.map(item => item.id as number)
    //抽屉显示出来
    drawer1.value = true
  }
}
...
const handleCheckAllChange = (val: boolean) => {
  userRoleList.value = val ? allRoleIdList.value : []
  //不确定样式
...
const saveRole = async () => {
  ...
  let data: SetRoleData = {
    ...
    roleIdList: userRoleList.value as number[],
  }
